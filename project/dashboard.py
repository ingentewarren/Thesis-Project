...
class MainFrame:
    def __init__(self, dash):
        ...
        def default():
            ...
            dashboard_icon_bg = Button(f2, image=dashboard_icon, border=0, command=default_dashboard_page)
            dashboard_icon_bg.place(x=100, y=440)

            classroom_icon = ImageTk.PhotoImage(classroom.resize(size=(200, 80)))
            classroom_icon_bg = Button(f2, image=classroom_icon, border=0, command=default_classroom_page)
            classroom_icon_bg.place(x=400, y=440)

            account_icon = ImageTk.PhotoImage(account.resize(size=(200, 80)))
            account_icon_bg = Button(f2, image=account_icon, border=0, command=default_account_page)
            account_icon_bg.place(x=700, y=440)

        def default_dashboard_page():
            ...
            def mobile_users():
                ...
                cursor = conn.cursor()
                cursor.execute("SELECT COUNT(*) FROM mobile_user")
            ...
            num_mobile_user = mobile_users()
            Label(box1, text=f": {num_mobile_user}", font=('Inter', 38, 'bold'), bg='#2B3467', fg='white').place(x=10, y=75)
        ...
        def default_account_page():
            ...
            def add():
                ...
                if surname != '' and firstname != '' and username !='' and password !='':
                    ...
                    try:
                        mycursor.execute("INSERT INTO mobile_user (surname, firstname, username, password, account_type) VALUES (%s, %s, %s, %s, %s) ",
                            (surname, firstname, username, password, account_type))
                        conn.commit()
                        ...
                        mycursor.execute("SELECT id, surname, firstname, username, password, account_type FROM mobile_user")
                        records = mycursor.fetchall()
                        print(f"adding records with surname {surname}")
                        print(records)
                        listview.delete(*listview.get_children())
                        for i, (id, surname, firstname, username, password, account_type) in enumerate(records, start=1):
                            listview.insert("", "end", values=(id, surname, firstname, username, password, account_type))
            ...
            def update():
                ...
                try:
                    mycursor.execute("UPDATE mobile_user SET firstname = %s, username = %s, password = %s, account_type = %s WHERE surname = %s",
                        (firstname, username, password, account_type, surname))
                    conn.commit()
            ...
            def show():
                ...
                record = mycursor.fetchall()
                conn.close()  
                for i, (id, surname, firstname, username, password, account_type) in enumerate(record, start=1):
                        listview.insert("", "end", values=(id, surname, firstname, username, password, account_type))  
